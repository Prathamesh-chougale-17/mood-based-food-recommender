...
res_data = res_data.loc[
    (res_data["Country Code"] == 1) & (res_data["City"] == "New Delhi"), :
]
...
def find_restaurant():
    ...
    for item in result:
        restaurants = (
            res_data[
                res_data.Cuisines.str.contains(food_to_cuisine_map[item], case=False)
            ]
    ...
    return render_template(
        ...
        result=result_str,
        mood=mood,
        restaurants1=restaurants_list[:3],
        restaurants2=restaurants_list[3:6],
        restaurants3=restaurants_list[6:],
    )
